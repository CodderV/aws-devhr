...
A concrete workflow I owned is **sales tax exemption**. The customer starts the request in the QuickBooks plugin (201, does not wait). Certificate verification may go through **Avalara**; a **Kubernetes CronJob** then runs an in-process **orchestration Saga**: lock request, fetch account, build payload, create customer downstream, update our record, update IFS/TES status. A **processing context** stores the stage. If a step fails we stop, **compensate** earlier steps that need it, update retry/status, and continue the **next record**. That is not Kafka and not one distributed database transaction.
